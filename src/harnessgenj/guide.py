...
HarnessGenJ 提供多种使用方式，你可以选择最适合你的方式：
...
"""
    ...
    def __init__(self, config_path: str | None = None) -> None:
        ...
        Args:
            config_path: 配置文件路径，默认为当前目录下的 .harnessgenj/config.json
        """
        if config_path:
            self.config_path = config_path
        else:
            # 默认配置路径
            self.config_path = os.path.join(os.getcwd(), ".harnessgenj", "config.json")
    ...
    def start(self, harness_instance: Any = None) -> ProjectConfig:
        ...
        """
        print("\n" + "=" * 60)
        print("欢迎使用 HarnessGenJ - Python Harness for AI Agents")
        print("=" * 60)
        ...
        project_description = self._prompt_input(
            "项目描述（一句话介绍）",
            default="一个使用 HarnessGenJ 框架的项目",
        )
        ...
        print("【步骤 2】团队角色配置")
        print("-" * 40)
        print("HarnessGenJ 提供以下角色，你可以自定义名称或使用默认值：")
